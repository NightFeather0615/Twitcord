...
client = commands.Bot(
    ...
    activity=discord.Activity(type=discord.ActivityType.watching, name=f"🕊️ | tc!link"),
)
...
def get_id_from_url(tweet_url):
    if (
        any(word in tweet_url for word in twitter_urls)
        and tweet_url.find("status") != -1
    ):
...
async def link_account(user, catch_message=None):
    ...
    async for msg in user.history():
        if (
            ...
            and msg.content.startswith("Twitter User Access Token")
            or msg.content.startswith("Twitter User Token")
        ):
            await msg.edit(
                content="[Cancelled] Twitter User Access Token\n`[Access Token cancelled]`\n`[Access Token Secret cancelled]`"
            )
    ...
    try:
        ...
        try:
            pin_code_msg = await client.wait_for(
                event="message", check=check, timeout=75.0
            )
            ...
            try:
                ...
                await user.send(embed=embed)
                token_msg = await user.send(
                    f"Twitter User Access Token\n||`{auth.access_token}`||\n||`{auth.access_token_secret}`||"
                )
                await token_msg.pin()
...
#         await ctx.send(
#             f"https://twitter.com/{tweet.includes['users'][0].username}/status/{new_tweet.data['id']}"
#         )
...
async def ping_calc(ctx, msg, index):
    ...
    for i in range(1, index + 1):
        ...
        loading_animation = (
            f"Tracking bot latency{loading_dot}{progress} {round(i/index*100, 1)}%"
        )
    ...
    await msg.delete()
    embed = discord.Embed(
        title=f"📑 Latency record from {start_time} to {end_time}",
        description=f"Max: {max_ping} ms | Min: {min_ping} ms | Avg: {avg_ping} ms",
        color=0x3983F2,
    )
...
async def get_twitter_client(user, notify: bool):
    pins = await user.pins()
    link_notify_embed = discord.Embed(
    ...
    if (
        len(pins) == 0
        or pins[0].content.startswith("Twitter User Access Token") == False
    ) and notify == True:
        try:
...
async def on_raw_reaction_add(payload):
    ...
    if channel != None:
        ...
        if (
            any(word in message.content for word in twitter_urls)
            and get_id_from_url(message.content) != None
                ...
                if str(payload.emoji) == "📡":
                    try:
                        tweet = twitter_client.get_tweet(
                            get_id_from_url(message.content), expansions="author_id"
                        )
...
async def on_message(message):
    if (
        any(word in message.content for word in twitter_urls)
        and get_id_from_url(message.content) != None
        and isinstance(message.channel, discord.DMChannel) == False
    ):
        for i in ["📡", "🔁", "❤️"]:
            try:
...
async def ping(ctx, index=None):
    ...
    if type(index) == int:
        if 0 < index <= 60:
            ...
            embed = discord.Embed(
                title="⚠️ Command Failed",
                description="Index out of range(1~60).\n`tc!ping <index(1~60)>`",
                color=0xECA42C,
            )
